    ...
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      ...
      {footerLinks.map((footerLinks) => (
        <div key={footerLinks.key} className="flex flex-col ss:my-0 my-4 min-w-[150px]">
              <h4 className="font-poppins fot-medium text-[18px] leading-[27px] text-white">
                {footerLinks.title}
              </h4>
              <ul>
                {footerLinks.links.map((link, index) => (
                    <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimbWhite hover:text-secondary cursor-pointer`}
                    >
                      {link.name}
                    </li>
                ))} 
